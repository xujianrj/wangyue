...
- (void)getReady:(id)data {
    ...
    JackRESTArtist *art = [[JackRESTArtist alloc] init:[PreferencesHelper statusNetUserName]
                                                    ip:currentHostIP roomName:kRoomName];
    [JackRESTArtist createArtist:art];
    // Note:Change status to 1 as ready
...
- (void)getServerChanelList:(NSTimer *)timer {
    ...
    if (message == nil) {
        ...
        BOOL isIPv6=[ipAddressVersion isEqualToString:@"IPv6"];
        ...
        //Note: Change status to 2  as Sever created  SERVER CREATED:=2
        [self appendOutputTextLine:@"Connecting server to change status to SERVER CREATED."];
...
- (void)getClientChanelList:(NSTimer *)timer {
    ...
    if (message == nil) {
        ...
        //Note:CLIENT CREATED=3
        [self appendOutputTextLine:@"Connecting server to change status to CLIENT CREATED."];
